...
%% Project 3. (part 1)
function [exist]= checkforexist(x,y,res)
% x=x-1;y=y-1;
...
v= ((x-180)*(x-180)+ (y-120)*(y-120) - (15.5+res)*(15.5+res)); %% Used to calculate if point is inside circle or not.
...
if (x>=0 && x<=250-res && y>=0 &&y<=150-res)
    % collision avoidance with sqaure
       if( x>=55-res && x<=105+res && y>67.5-res && y<112.5+res )
           exist = false;                           
