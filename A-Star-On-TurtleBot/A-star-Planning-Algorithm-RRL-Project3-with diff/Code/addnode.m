...
%% Project 3. (part 3)
function [X,Pnode,ncount,pcount,open,cost,c2c,c,vel]=addnode(xy,X,closelist,Pnode,xgoal,ygoal,ncount,c2ccount,count,pcount,open,cost,c2cdiff,c2c,c,resolution,vel,v)
...
if (checkforexist(xy(1),xy(2),resolution))                  %check for collision
   if (~ismember([xy(1) xy(2) xy(3)],closelist,'rows'))      %check if node not in close list  
        
       % Calculating Cost to come 
        c2c(pcount) = c2c(c2ccount) +c2cdiff; 
        ...
        if (ismember([xy(1) xy(2) xy(3)],open,'rows'))      
%             [~,indexX]= ismember([xy(1) xy(2)],X,'rows');
            [~,indexo]= ismember([xy(1) xy(2) xy(3)],open,'rows');
