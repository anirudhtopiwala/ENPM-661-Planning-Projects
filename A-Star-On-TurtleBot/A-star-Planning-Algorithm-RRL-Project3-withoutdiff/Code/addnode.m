...
if (checkforexist(xy(1),xy(2),resolution))         %check for collision
   if (~ismember([xy(1) xy(2)],closelist,'rows')) %check if node not in close list  
       ...
       if(nodenum==1)
              c2c(pcount) = c2c(c2ccount) +1*resolution;  % Resolution is multiplied here, because with change in resolution, cost to come to take one step will also change.
        else
              c2c(pcount)= c2c(c2ccount) +1.4*resolution;  % Resolution is multiplied here, because with change in resolution, cost to come to take one step will also change.
        end
